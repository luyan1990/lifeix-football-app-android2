 ...
 * 裁判员 -- 五人制页面 适配器
 */
public class FivePepleListAdapter extends BaseAdapter implements StickyListHeadersAdapter {



    private  ArrayList<ArrayList<RefereesBean.CategoryBean.RefereeBean>> list;

    private ArrayList<RefereesBean.CategoryBean.RefereeBean> WuRenZhiZuQiuNan;
    private ArrayList<RefereesBean.CategoryBean.RefereeBean> WuRenZhiZuQiuNv;
    private ArrayList<RefereesBean.CategoryBean.RefereeBean> zong;

    public FivePepleListAdapter(ArrayList<ArrayList<RefereesBean.CategoryBean.RefereeBean>> list) {

        this.list = list;
        WuRenZhiZuQiuNan = list.get(4);
        WuRenZhiZuQiuNv = list.get(5);

        zong = new ArrayList<>();
        zong.addAll(WuRenZhiZuQiuNan);
        zong.addAll(WuRenZhiZuQiuNv);
    }

    private String getHeadTittle(int position){
        String tittle="";
        if(position<WuRenZhiZuQiuNan.size()) {
            tittle="五人制足球(男)";
        }else {
            tittle="五人制足球(女)";
        }
