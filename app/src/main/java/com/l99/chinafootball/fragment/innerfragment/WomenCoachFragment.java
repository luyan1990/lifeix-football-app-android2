...
import com.l99.chinafootball.adapter.WomenCoachGridViewAdapter;
...
public class WomenCoachFragment extends BaseFragment implements IView.ViewPro<Map<String,ArrayList<NationalCoachBean.CategoryBean>>> {

    private final CoachlistPresenter presenter;
    private XRefreshView xRefreshView;
    private StickyGridHeadersGridView stickyGridHeadersGridView;

    private WomenCoachGridViewAdapter mAdapter;

    private List<NationalCoachBean.CategoryBean.CoachesBean> coachesBeans=new ArrayList<>();

    @Override
    public String getTAG() {
        return WomenCoachFragment.class.getSimpleName();
    }

    public WomenCoachFragment(Context context) {
        super(context);
    ...
    public void setData(Map<String, ArrayList<NationalCoachBean.CategoryBean>> data) {
        final ArrayList<NationalCoachBean.CategoryBean> women = data.get("女足教练");

        mAdapter = new WomenCoachGridViewAdapter(context,women);
        stickyGridHeadersGridView.setAdapter(mAdapter);

        for(int i = 0; i < women.size(); i++) {
            NationalCoachBean.CategoryBean categoryBean = women.get(i);
